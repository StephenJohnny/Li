...
c8x = linspace(0.00, 0.0165);
% (x-r)^2 + y^2 = R^2
c8y = sqrt(0.05^2 - (c8x - 0.033).^2);
plot(c8x, c8y,'b--','linewidth',0.5);

c9x = linspace(0.00, 0.030);
% (x-r)^2 + y^2 = R^2
c9y = sqrt(0.04^2 - (c9x - 0.033).^2);
plot(c9x, c9y,'b--','linewidth',0.5);

c10x = linspace(0.003, 0.0408);
% (x-r)^2 + y^2 = R^2
c10y = sqrt(0.03^2 - (c10x - 0.033).^2);
plot(c10x, c10y,'b--','linewidth',0.5);

c11x = linspace(0.013, 0.048);
% (x-r)^2 + y^2 = R^2
c11y = sqrt(0.02^2 - (c11x - 0.033).^2);
plot(c11x, c11y,'b--','linewidth',0.5);

c12x = linspace(0.023, 0.043);
% (x-r)^2 + y^2 = R^2
c12y = sqrt(0.01^2 - (c12x - 0.033).^2);
plot(c12x, c12y,'b--','linewidth',0.5);

% 画内部绿色直线段
l_alphas = cticks_([2 3 4 5 6 8 9 10]);
for alpha = l_alphas
    [x0,y0]=getCirclePoint(pi/2-alpha,0.05);
    line([0,x0], [0,y0],'LineStyle','--','Color',[0 0.5 0]);
end

% Part 3 画内部点
plot(0.033, 0.000, '.','MarkerSize',12,'LineWidth',5.0,'Color','b');
text(0.030, -0.002,'REF','FontName','Times New Roman');
...
for i = 1:length(m)
    p(i) = plot(m(i),n(i),color_type{i},'MarkerSize',masz(i));
...
global R
    ...
    for alpha = cticks_
        ct = ct+1;
        [x0,y0] = getCirclePoint(pi/2-alpha, R+0.0018);
        text(x0-0.001,y0,cticklabels_{ct},'FontName','Times New Roman');
    end
end
% cticks在1/4圆弧外围绘制刻度线
...
global R
    for alpha = cticks_
        CycleTickline(pi/2 - alpha, R, 0.0008)
    end
end
% 在有标签的刻度线中间加入刻度线
...
global R
    for alpha = cminorticks_
        CycleTickline(pi/2 - alpha, R, 0.0004)
    end
end
